...
class TUITest extends AnyWordSpec with Matchers {
  ...
  "A TUI" should {
    ...
    "start the game and create a new game with valid player input" in {
      ...
      // Valid input test
      val namesIO = "Player1, Player2, Player3, Player4"
      ...
      assert(controller.gameState.players.map(_.name).equals(Seq("Player1", "Player2", "Player3", "Player4")))
    }
    ...
    "reject invalid player input" in {
      ...
      val names = "Player1" // Invalid input
